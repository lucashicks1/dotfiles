-- Have the servers and options at the top
local servers = {
  bashls = {
    filetypes = { 'bash', 'sh' },
  },
  ...
  config = function()
    vim.api.nvim_create_autocmd('LspAttach', {
      ...
      callback = function(event)
        ...
        map('g.', vim.lsp.buf.code_action, '[G]oto [C]ode [A]ction', { 'n', 'x' })
        ...
        map('<leader>sd', vim.diagnostic.open_float, '[S]how Line [D]iagnostic')

        -- Misc / debug
        map('<leader>li', '<cmd>LspInfo<cr>', 'LSP [I]nfo')
        map('<leader>lr', ':LspRestart<cr>', 'LSP [R]estart')
        map('<leader>ll', ':LspLog<cr>', 'LSP [L]og')
